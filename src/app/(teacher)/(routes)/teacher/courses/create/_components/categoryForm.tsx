...
const CategorySection = ({ initialValue, courseId, options }: CategorySectionProps) => {
  ...
  const selectedOptions = options.find((option) => option.value === initialValue);
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      // console.log(data);
      const res = await axios.patch(`/api/courses/create/${courseId}`, {category: data.categoryId});
    ...
    <div className="mt-4 rounded-lg bg-slate-100 px-4 py-2">
      ...
      <div className="mt-6">
        {!isEdit && (
          ...
          >
            {!initialValue ||
              (initialValue === "undefined" && "no category")}
